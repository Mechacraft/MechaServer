...
import { Vector as MCVector, BlockPermutation } from "@minecraft/server";
...
function canSnowOn(block) {
    ...
    if (isBlocked)
        isBlocked && (isBlocked = !!dimension.getBlockFromRay(location.add([-0.49, 0, 0]), MCVector.down, solidTest));
    if (isBlocked)
        isBlocked && (isBlocked = !!dimension.getBlockFromRay(location.add([0.49, 0, 0]), MCVector.down, solidTest));
    if (isBlocked)
        isBlocked && (isBlocked = !!dimension.getBlockFromRay(location.add([0, 0, -0.49]), MCVector.down, solidTest));
    if (isBlocked)
        isBlocked && (isBlocked = !!dimension.getBlockFromRay(location.add([0, 0, 0.49]), MCVector.down, solidTest));
    return isBlocked;
}
            ...
            for (const block of blocks) {
                if (block.typeId.match(waterMatch)) {
                    ...
                    if (args.has("s") && Math.random() < 0.4) {
                        ...
                        const prevHeight = perm.getState("height");
                        perm = perm.withState("height", Math.min(prevHeight + 1, 7));
                        block.setPermutation(perm);
                        if (perm.getState("height") != prevHeight)
                            changed++;
                    }
                }
